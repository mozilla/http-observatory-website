(function() {
    ...
    function handleScanResults(scan) {
        ...
        window.Observatory.state.scan = scan;
        ...
        if (scan.error) {
            ...
            if (scan.error === 'recent-scan-not-found') {
                ...
                submitScanForAnalysisXHR(window.Observatory.hostname, success, failure, 'POST', false, true);
                return false;
            }
        ...
        if (retry) {
            ...
            if (scan.state === 'FINISHED') {
                ...
                $.ajax({
                    ...
                    url: window.Observatory.api_url + 'getScanResults?scan=' + scan.scan_id.toString()
                });
            }
        }
    }
    ...
    function insertScanResults(scan, results) {
        ...
        scan['hostname'] = window.Observatory.hostname;

        // stuff both scan and state into the HTTPObs object
        window.Observatory.state.scan = scan;
        window.Observatory.state.results = results;

        // add a test duration
        window.Observatory.state.scan.test_duration = (
            moment.utc(window.Observatory.state.scan.end_time, 'ddd, DD MMM YYYY HH:mm:ss zz') -
            moment.utc(window.Observatory.state.scan.start_time, 'ddd, DD MMM YYYY HH:mm:ss zz')) / 1000;

        // convert things to local time
        window.Observatory.state.scan.start_time_l = window.toLocalTime(window.Observatory.state.scan.start_time, 'ddd, DD MMM YYYY HH:mm:ss zz');
        window.Observatory.state.scan.end_time_l = window.toLocalTime(window.Observatory.state.scan.end_time, 'ddd, DD MMM YYYY HH:mm:ss zz');

        // enable the rescan button if the test was over 315 seconds ago, otherwise enable it at that point
        var lastScanDelta = moment() - moment.utc(window.Observatory.state.scan.end_time, 'ddd, DD MMM YYYY HH:mm:ss zz');
        ...
        window.insertGrade(scan.grade, 'scan');
        window.insertResults(scan, 'scan');
        ...
        keys = Object.keys(window.Observatory.state.scan.response_headers);
        for (var i in keys) {
            key = keys[i];
            ...
            $('<td></td>').text(window.Observatory.state.scan.response_headers[key]).appendTo(tr);
    ...
    function loadScanResults() {
        submitScanForAnalysisXHR(window.Observatory.hostname, handleScanResults, displayError);
    }
    ...
    function enableInitiateRescanButton() {
        $('#force-rescan').removeClass('disabled').on('click', function() {
            ...
            window.loadTLSObservatoryResults(true);
            submitScanForAnalysisXHR(window.Observatory.hostname, reload, reload, 'POST', true, true);
            return false;
        });
    }
    ...
    function insertResultTable(data, title, id, alert) {
        ...
        if (id === 'totalresults') {
            ...
            for (var i = 0; i < window.Observatory.grades.length; i++) {
                tbody.append('<tr><td>' + window.Observatory.grades[i] + '</td><td class="text-right">' + data[window.Observatory.grades[i]] + '</td>');
                sum += data[window.Observatory.grades[i]];
            }
    ...
    function submitScanForAnalysisXHR(hostname, successCallback, errorCallback, method, rescan, hidden) {
        ...
        var rescan = typeof rescan === 'undefined' ? false : rescan;
        var hidden = typeof hidden === 'undefined' ? false : hidden;
        ...
        var config = {
            ...
            url: window.Observatory.api_url + 'analyze?host=' + hostname
        };
    ...
    function submitScanForAnalysis() {
        ...
        window.Observatory.hostname = hostname;
        ...
        if (rescan) {  // if they've set rescan, we'll poke the TLS Observatory now
            window.loadTLSObservatoryResults(rescan, true);
        }
    ...
    if (document.domain !== 'observatory.mozilla.org') {
        ...
        if (window.location.pathname.indexOf('/analyze.html') !== -1) {
            ...
            window.Observatory.hostname = window.getQueryParameter('host');
            ...
            loadScanResults();
            window.loadTLSObservatoryResults();

            // let's check the third parties if requested
            if (window.getQueryParameter('third-party') === 'false') {
                $('#third-party-tests').remove();
                $('#third-party-tests-page-header').remove();
            } else {
                // loadSafeBrowsingResults();
                window.loadHSTSPreloadResults();
                // loadHTBridgeResults();
            window.loadSecurityHeadersIOResults();
            window.loadSSLLabsResults();
            window.loadTLSImirhilFrResults();
            ...
            retrieveResultTable('Overall Results', window.Observatory.api_url + 'getGradeDistribution', 'totalresults', 'info');
            retrieveResultTable('Recently Scanned', window.Observatory.api_url + 'getRecentScans?num=14', 'recentresults', 'warning');
            retrieveResultTable('Recent Best', window.Observatory.api_url + 'getRecentScans?min=90&num=14', 'goodresults', 'success');
            retrieveResultTable('Recent Worst', window.Observatory.api_url + 'getRecentScans?max=20&num=14', 'badresults', 'danger');
        }
    }
