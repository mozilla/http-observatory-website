(function() {
    ...
    window.insertGrade = function (grade, id) {
        ...
        switch (grade) {
            case 'check-mark':
                letter = window.Observatory.utils.character_mappings.checkmark;
                dom_container.toggleClass('grade-a');
                break;
            case 'up-arrow':
                letter = window.Observatory.utils.character_mappings.uparrow;
                dom_container.toggleClass('grade-a');
                break;
            case 'x-mark':
                letter = window.Observatory.utils.character_mappings.xmark;
            ...
            case 'Old (Backwards Compatible)':
                ...
                break;
            case 'Intermediate':
                letter = window.Observatory.utils.character_mappings.latini;  // latin capital letter i
    ...
    window.insertResults = function (results, id) {
    ...
    window.errorResults = function (error, id) {
    ...
    window.linkify = function (url) {  // take a link and return an a href
    ...
    window.listify = function (list, force) {  // take an array and turn it into an unordered list, if it's > 1 item
    ...
    window.tableify = function (list, table_id) {  // take a list of lists and push it into an existing table
    ...
    window.toLocalTime = function (timeString, format) {
    ...
    window.getQueryParameter = function (param) {
